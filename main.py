...
def load_docs(query):
    with st.spinner(text="Loading and indexing the docs – hang tight!"):
        ...
        snowflake_password = st.secrets["connections"]['snowpark']['password']
        snowflake_account = st.secrets["connections"]['snowpark']['account']
        snowflake_database = st.secrets["connections"]['snowpark']['database']
        snowflake_schema = st.secrets["connections"]['snowpark']['schema']
